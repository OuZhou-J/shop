...
import com.shop.pojo.CartInstrumentPair;
...
public class ShoppingCartServiceImpl implements com.shop.service.ShoppingCartService {
    ...
    public void addShoppingCart(String username, int instrument_id, int quantity, int shipping_mode, BigDecimal money, BigDecimal weight) {
        ...
        ShoppingCart check_ShoppingCart = shoppingCartMapper.findShoppingCartByInstrument_idAndUsername(instrument_id,name);
        if(check_ShoppingCart != null){
            ...
            if(check_ShoppingCart.getShippingMode()==shipping_mode){
    ...
    @Override
    public List<CartInstrumentPair> findShoppingCartByUsername(String username) {
        List<ShoppingCart> cart=shoppingCartMapper.findShoppingCartByUsername(username);
        List<CartInstrumentPair> cartInstrumentPairs = new ArrayList<>();
        for(int i=0;i<cart.size();i++){
            System.out.println(cart.size());
            System.out.println(cart.get(0));
            System.out.println(cart.get(i).getId());
            CartInstrumentPair cartInstrumentPair = new CartInstrumentPair(cart.get(i),musicalInstrumentsService.getMusicalInstrumentsById(cart.get(i).getInstrumentId()));
            cartInstrumentPairs.add(cartInstrumentPair);
        }
        return cartInstrumentPairs;
    }
